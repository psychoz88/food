...
function calc() {
  ...
  function initLocalSettings(selector, activeClass) {
    // функция которая ставит классы активности елементам которые в localStorage
  ...
  function getDynamicInformation(selector) {
    ...
    input.addEventListener('input', function () {
      ...
      switch (input.getAttribute('id')) {
        // нужно проверить id
        case 'height':
          // если действительно рост то
          height = +input.value;
          break;

        case 'weight':
          // если действия происходят в весе
          weight = +input.value;
          break;

        case 'age':
          // если в графе возраст
          age = +input.value;
          break;
      }
  ...
  (0,_services_services__WEBPACK_IMPORTED_MODULE_3__.getResource)('http://localhost:3000/menu').then(function (data) {
    data.forEach(function (_ref) {
      var img = _ref.img,
          ...
          descr = _ref.descr,
          price = _ref.price;
      // деструктуризация объекта - когда из объекта вытаскиваем отдельные свойства в качестве отдельной переменной
...
function forms(formSelector, modalTimerId) {
  ...
  function bindpostData(form) {
    ...
    form.addEventListener("submit", function (e) {
      e.preventDefault();
      var statusMessage = document.createElement("img"); // добавляет новый елемент: при нажатии на кнопку появляетс спиннер
      ...
      var formData = new FormData(form); // formData - это специальный объект который позволяет в форме ключ-значение быстро сформировать все данные которые заполнил пользователь. В (form) пишем ту форму из которой нам надо собрать данные
      // В формате JSON

      var json = JSON.stringify(Object.fromEntries(formData.entries())); // сначала берем formData которая собрала все данные с формы, превращаем с помощью entries ее в матрицу(массив массивов), потом превращаем в классический объект(fromEntries), потом его прекращаем в JSON

      (0,_services_services__WEBPACK_IMPORTED_MODULE_9__.postData)("http://localhost:3000/requests", json) //.then(data => data.text()) // чтобы точно понимать какой нам прийдет ответ. Там точно не "JSON поэтому в текстовом формате пусть"
      .then(function (data) {
...
function modal(triggerSelector, modalSelector, modalTimerId) {
  ...
  modalTrigger.forEach(function (btn) {
    ...
    }); // по правилу нельзя вызывать функцию в слушателе потому что при клике она сама запустится. Это обходиться с помощью () =>
  });
...
function slider(_ref) {
  ...
  var slides = document.querySelectorAll(slide),
      ...
      // окошко через которое видно слайды
  ...
  next.addEventListener('click', function () {
    if (offset == deleteNotDigits(width) * (slides.length - 1)) {
      // последний слайд вычисляется по такой формуле
      offset = 0; // +width.slice(0, width.length - 2) - проставить везде где такое надо то есть всего в трёх местах
      // +width.replace(/[^\d.]/g, '')
    } else {
      offset += deleteNotDigits(width); // offset += +width.slice(0, width.length - 2);
    }

    slidesField.style.transform = "translateX(-".concat(offset, "px)"); // это наше 'колесо' которое переключает слайды
  ...
  dots.forEach(function (dot) {
    dot.addEventListener('click', function (e) {
      ...
      slideIndex = slideTo;
      offset = deleteNotDigits(width) * (slideTo - 1); // offset = +width.slice(0, width.length - 2) * (slideTo - 1);
...
function timer(id, deadline) {
  ...
  function getTimeRemaining(endtime) {
    // вычесляет сколько времени осталось
